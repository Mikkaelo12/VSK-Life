...
SimpleChatbot.prototype._templateBtn = function (botIndex, content) {
	return (
		'<button class="btn" type="button" data-bot-index="' +
		botIndex +
		'">' +
		content +
		'</button>'
...
SimpleChatbot.prototype._outputContent = function (interval) {
	...
	var fn2 = function () {
		if (_this._getData('human', humanIds[0]).content === '') {
			...
			_this._$root.querySelector('.chatbot__input').dataset.name =
				_this._getData('human', humanIds[0]).name
			_this._$root.querySelector('.chatbot__submit').disabled = true
			_this._$root.querySelector('.chatbot__input').focus()
			_this._$root.querySelector('.chatbot__submit').dataset.botIndex =
				_this._getData('human', humanIds[0]).bot
		} else {
			...
			var $humanContentWrapper = _this._template(
				'human',
				$humanContent.join('')
			)
...
SimpleChatbot.templateItem = function (replicas) {
	let html =
		'<div class="chatbot__item chatbot__item_{{type}}"><div class="chatbot__content chatbot__content_{{type}}{{state}}">{{content}}</div></div>'
...
SimpleChatbot.resetTemplate = function () {
	...
	$root.innerHTML =
		'<div class="chatbot__title"><span class="chatbot__close"></span>Чат-бот VSK life<span class="chatbot__reset"></span></div><div class="chatbot__wrapper"><div class="chatbot__items"></div></div><div class="chatbot__footer"><input class="chatbot__input" type="text" disabled><button class="chatbot__submit" type="button" disabled></button></div>'
}
